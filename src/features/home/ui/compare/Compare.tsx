...
export const Compare = ({
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      if (!autoplay && e.touches[0] !== undefined) {
  ...
  return (
    ...
    >
      <AnimatePresence initial={false}>
        <motion.div
          className="absolute top-0 z-30 m-auto h-full bg-gradient-to-b from-transparent from-[5%] via-indigo-500 to-transparent to-[95%]"
          style={{
            left: `${sliderXPercent}%`,
            top: "0",
            zIndex: 40,
      ...
      <div className="pointer-events-none relative z-20 h-full w-full overflow-hidden bg-[]">
        <AnimatePresence initial={false}>
          {firstImage ? (
            <motion.div
              className={cn(
            ...
            >
              <Image
                ...
                priority // Optionally add `priority` for LCP optimization if needed
              />
            </motion.div>
          ) : null}
        </AnimatePresence>
      ...
      <AnimatePresence initial={false}>
        {secondImage ? (
          <motion.img
            className={cn(
              "absolute left-0 top-0 z-[19] h-full w-full select-none rounded-2xl bg-black",
              secondImageClassname,
            )}
            alt="second image"
            src={secondImage}
